...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<h4 class="panel-title">
			待办任务&nbsp;&nbsp;&nbsp;总数：<span style="color: red">${LoansOrderSum}
		</h4>
		<p>显示正在要处理审核的待办任务</p>
	</div>
		...
		<table class="table table-bordered table-striped table-bordered">
			...
			<tbody>
				<c:forEach var="item" items="${orderNodeList}" varStatus="status">
					<tr>
						<%-- <td><a href="${ctx}/loansAssist?nodeStatusEq=${item.nodeStatus}" >查看</a></td>  --%>

						<td><c:choose>
								<c:when test="${item.nodeStatus eq -1 }">
									<a href="${ctx}/${item.auditUrl}">查看</a>
									<!--如果 -->
								</c:when>
