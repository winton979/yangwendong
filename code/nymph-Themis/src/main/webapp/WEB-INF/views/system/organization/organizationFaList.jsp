    ...
    <table id="shTable1" class="table table-striped table-bordered responsive dataTable no-footer dtr-inline" role="grid">
        <thead>
            <tr>
                ...
                <tags:sort curretOrder="${criteria.orderByClause}" fieldName="ORG_NAME" target="listDiv" fieldTitle="车行名称"/>
           <%-- <tags:sort curretOrder="${criteria.orderByClause}" fieldName="ORG_PARENT_SID" target="listDiv" fieldTitle="上级4S店"/> --%>
                ...
                <th>联系电话</th>
                <th>状态</th>
                <tags:sort curretOrder="${criteria.orderByClause}" fieldName="ENABLED" target="listDiv" fieldTitle="创建时间"/>
        ...
        <tbody>
          <c:forEach var="organ" items="${resultList}" varStatus="sta">
            <tr>
            	<td>
                	<ms:hasAnyPermission name="superAdmin,organization:update,organization:fa">
                		<a href="${ctx}/organization/faView/${organ.sid}" class="btn btn-primary" data-toggle="modal" data-target=".bs-organization-edit-modal">查看</a>
                		<c:if test="${organ.status ne 1 }">
                			<c:if test="${organ.auditStatus == 2}">
                				<a href="${ctx}/organization/faUpdate/${organ.sid}" class="btn btn-warning" data-toggle="modal" data-target=".bs-organization-edit-modal">编辑</a>
                			</c:if>
                		</c:if>
                ...
                <td>${organ.contactTel}</td>
	             ...
	             <c:if test="${organ.status == 2 and organ.auditStatus eq 1}">
	              	<span class="badge badge-primary">已提交审核</span>
	             </c:if>
	             <c:if test="${organ.status == 2 and organ.auditStatus eq 2}">
	              	<span class="badge badge-warning">${auditStatus[organ.status]}</span>
	             </c:if>
	             
	               <c:if test="${organ.status == 2 and organ.auditStatus eq 0}">
		            	<span class="badge badge-danger">${auditStatus[organ.status]}</span>
		            </c:if>
