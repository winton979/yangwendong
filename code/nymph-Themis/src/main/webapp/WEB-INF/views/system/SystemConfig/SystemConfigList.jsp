    ...
    <table id="shTable2" class="table table-striped table-bordered table-condensed responsive dataTable no-footer dtr-inline" role="grid">
        <thead>
            <tr>
                ...
                <th>操作</th>
				<th><%=SystemConfig.ALIAS_CONFIG_CODE%></th>
				<th><%=SystemConfig.ALIAS_CONFIG_NAME%></th>
				<th>值</th>
            </tr>
        </thead>
        <tbody>
          <c:forEach var="item" items="${resultList}" varStatus="status">
            <tr>
            	...
            	<td>
                   ...
                   <ms:hasAnyPermission name="superAdmin,systemConfig:delete">
	                   <a href="javascript:void(0);" onclick="javascript:deleteItem('${ctx}/systemConfig/delete/${item.sid}');" class="btn btn-danger" id="growl-danger">删除</a>
                   </ms:hasAnyPermission>
                </td>
				<td><c:out value='${item.configCode}'/></td>
				<td><c:out value='${item.configName}'/></td>
				<td>
					<strong>配置值1:</strong>
					<c:if test="${!empty item.configValue}">
						<c:out value='${item.configValue}'/>
					</c:if>
					<c:if test="${empty item.configValue}">
						<span class="text-danger">未配置</span>
					</c:if>
					<%-- <strong>配置值2:</strong></br>
					<c:if test="${!empty item.configValue2}">
						<c:out value='${item.configValue2}'/></br>
					</c:if>
					<c:if test="${empty item.configValue2}">
						<span class="text-danger">未配置</span></br>
					</c:if>
					<strong>配置值3:</strong></br>
					<c:if test="${!empty item.configValue3}">
						<c:out value='${item.configValue3}'/></br>
					</c:if>
					<c:if test="${empty item.configValue3}">
						<span class="text-danger">未配置</span></br>
					</c:if> --%>
